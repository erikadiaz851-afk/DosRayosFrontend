...
function Bolts() {
  const b = { display:'block',width:10,height:24,background:C.yellow,clipPath:'polygon(60% 0%,100% 0%,40% 50%,100% 50%,0% 100%,60% 100%,20% 50%,0% 50%)' };
...
export default function RegisterPage({ onSwitch }) {
  ...
  async function handleSubmit(e) {
    ...
    setLoading(true);
    try { await register(name, email, pass); }
    catch (err) { setError(err.message); }
    finally { setLoading(false); }
  }

  const input = {width:'100%',padding:'10px 14px',background:'#0d0d0d',border:`1px solid ${C.border}`,borderRadius:8,color:C.text,fontSize:13,outline:'none',fontFamily:'inherit'};
  const lbl   = {fontSize:11,color:C.muted,fontFamily:'monospace',letterSpacing:1,textTransform:'uppercase',display:'block',marginBottom:6};
    ...
    <div style={{minHeight:'100vh',background:C.bg,display:'flex',alignItems:'center',justifyContent:'center',fontFamily:'-apple-system,sans-serif',backgroundImage:'linear-gradient(rgba(255,255,255,0.012) 1px,transparent 1px),linear-gradient(90deg,rgba(255,255,255,0.012) 1px,transparent 1px)',backgroundSize:'40px 40px'}}>
      <div style={{background:C.surface,border:`1px solid ${C.border}`,borderRadius:16,padding:'36px 40px',width:'100%',maxWidth:400}}>
        <div style={{display:'flex',alignItems:'center',gap:10,marginBottom:28}}>
          <Bolts />
          <div>
            <div style={{fontSize:20,fontWeight:800,color:C.yellow}}>DOS <span style={{color:C.blue}}>RAYOS</span></div>
            <div style={{fontSize:10,color:C.muted,fontFamily:'monospace',letterSpacing:1.5}}>BRAND INTELLIGENCE</div>
          </div>
        </div>

        <h2 style={{fontSize:18,fontWeight:700,color:C.text,marginBottom:6}}>Crear cuenta</h2>
        <p style={{fontSize:12,color:C.muted,marginBottom:24}}>Únete al dashboard de Dos Rayos</p>

        <form onSubmit={handleSubmit}>
          {[['Nombre','text',name,setName,'Tu nombre'],['Email','email',email,setEmail,'tu@email.com']].map(([lbl2,type,val,set,ph])=>(
          ...
          <div style={{marginBottom:14}}>
            ...
            <input type="password" value={pass} onChange={e=>setPass(e.target.value)} placeholder="Mínimo 6 caracteres" required style={input}
              onFocus={e=>e.target.style.borderColor=C.yellow} onBlur={e=>e.target.style.borderColor=C.border}/>
          </div>
          <div style={{marginBottom:20}}>
            <label style={lbl}>Confirmar contraseña</label>
            <input type="password" value={pass2} onChange={e=>setPass2(e.target.value)} placeholder="Repite la contraseña" required style={input}
              onFocus={e=>e.target.style.borderColor=C.yellow} onBlur={e=>e.target.style.borderColor=C.border}/>
          </div>
        ...
        <p style={{textAlign:'center',fontSize:12,color:C.muted,marginTop:20}}>
          ¿Ya tienes cuenta?{' '}
          <span onClick={onSwitch} style={{color:C.blue,cursor:'pointer',textDecoration:'underline'}}>Iniciar sesión</span>
        </p>
      </div>
    </div>
  );
